...
$('.oe_website_purchase').each(function () {
    ...
    var $shippingDifferent = $("select[name='shipping_id']", oe_website_purchase);
    $shippingDifferent.change(function (event) {
        ...
        var $snipping = $(".js_shipping", oe_website_purchase);
    ...
    $(oe_website_purchase).on("change", 'input[name="add_qty"]', function (event) {
        ...
        openerp.jsonRpc("/purchase/get_unit_price", 'call', {'product_ids': product_ids,'add_qty': parseInt(qty)})
        .then(function (data) {
    ...
    $(oe_website_purchase).find(".oe_cart input.js_quantity").on("change", function () {
        var $input = $(this);
        ...
        openerp.jsonRpc("/purchase/get_unit_price", 'call', {
        ...
        .then(function (res) {
            ...
            openerp.jsonRpc("/purchase/cart/update_json", 'call', {
            'line_id': line_id,
    ...
    $('form.js_attributes input, form.js_attributes select', oe_website_purchase).on('change', function () {
        $(this).closest("form").submit();
    });

    // change price when they are variants
    $('form.js_add_cart_json label', oe_website_purchase).on('mouseup touchend', function (ev) {
        var $label = $(this);
    ...
    $('.css_attribute_color input', oe_website_purchase).on('change', function (ev) {
        ...
        for (var k in variant_ids) {
            if (_.isEmpty(_.difference(variant_ids[k][1], values))) {
                ...
                if (variant_ids[k][3]-variant_ids[k][2]>0.2) {
                    ...
                    $default_price.closest('.oe_website_purchase').removeClass("discount");
    ...
    $(oe_website_purchase).on('change', "select[name='country_id']", function () {
        var $select = $("select[name='state_id']");
        $select.find("option:not(:first)").hide();
        var nb = $select.find("option[data-country_id="+($(this).val() || 0)+"]").show().size();
        $select.parent().toggle(nb>1);
    });
    $(oe_website_purchase).find("select[name='country_id']").change();

    $(oe_website_purchase).on('change', "select[name='shipping_country_id']", function () {
        var $select = $("select[name='shipping_state_id']");
        $select.find("option:not(:first)").hide();
        var nb = $select.find("option[data-country_id="+($(this).val() || 0)+"]").show().size();
        $select.parent().toggle(nb>1);
    });
    $(oe_website_purchase).find("select[name='shipping_country_id']").change();
});
});
