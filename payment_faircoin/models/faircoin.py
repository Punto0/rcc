...
#        base_url = self.pool['ir.config_parameter'].get_param(cr, SUPERUSER_ID, 'web.base.url')
    ...
    _columns = {
        ...
        #'faircoin_use_ipn': fields.boolean('Use IPN', help='Faircoin Instant Payment Notification'),
        # Server 2 server
    ...
    """
    def _migrate_faircoin_account(self, cr, uid, context=None):

      cr.execute('SELECT id, faircoin_account FROM res_company')
      res = cr.fetchall()
      for (company_id, company_faircoin_account) in res:
          if company_faircoin_account:
              company_faircoin_ids = self.search(cr, uid, [('company_id', '=', company_id), ('provider', '=', 'faircoin')], limit=1, context=context)
              if company_faircoin_ids:
                self.write(cr, uid, company_faircoin_ids, {'faircoin_faircoin_address': company_faircoin_account}, context=context)
              else:
                  electrum_view = self.pool['ir.model.data'].get_object(cr, uid, 'payment_faircoin', 'faircoin_acquirer_button')
                  self.create(cr, uid, {
                        'name': 'Faircoin',
                        'provider': 'faircoin',
                        'faircoin_seller_account': company_faircoin_account,
                        'view_template_id': faircoin_view.id,
                    }, context=context)
      return True
    """
    ...
    def faircoin_form_generate_values(self, cr, uid, id, partner_values, tx_values, context=None):
        ...
        # ToDo: si la referencia está usada, incrementa uno y prueba de nuevo tx_value[reference]        
        ...
        if order_id:
            ...
            for order_line in order_lines:
                ...
                company = order_line.product_id.company_id
                faircoin_address = company.faircoin_account
